...
import { taogetCat, taogetList, taogetOne, taogetComment, taoaddComment, taoaddAnswer, taoaddZan } from '../../../services/API'
...
Page({
  ...
  sharezan(e) {
    ...
    taoaddZan({ article_id:id}).then(({ status, result, msg }) => {
      if (status == 1) { 
        this.taogetList(this.data.activeType);
      } else {
        app.wxAPI.alert(msg)
      }
    })
