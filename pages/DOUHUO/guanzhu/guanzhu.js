// pages/WEIdhuo/weidhuo/weidhuo.js
import { dhuogetCat, dhuogetList, dhuogetOne, dhuogetComment, dhuoaddComment, dhuoaddAnswer, dhuoaddZan } from '../../../services/API'
...
Page({
  ...
  sharezan(e) {
    ...
    dhuoaddZan({ article_id: id }).then(({ status, result, msg }) => {
      if (status == 1) {
        this.dhuogetList(this.data.activeType);
      } else {
        app.wxAPI.alert(msg)
      }
    })
  ...
  dhuogetList(e) {
    dhuogetList(e).then(({ status, result, msg }) => {
      if (status == 1) {
  ...
  onLoad: function (options) {
    ...
    dhuogetCat().then(({ status, result, msg }) => {
      ...
      if (status == 1) {
        ...
        WxParse.wxParse('article', 'html', result.data[0].content, this, 0);
      }
    })
    this.dhuogetList();
